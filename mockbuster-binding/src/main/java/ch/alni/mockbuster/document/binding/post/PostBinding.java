...
package ch.alni.mockbuster.document.binding.post;
...
import ch.alni.mockbuster.service.binding.Binding;
import ch.alni.mockbuster.service.binding.InvalidRequestException;

/**
 * Implementation for the POST binding.
 */
@Component
public class PostBinding implements Binding {
    ...
    public String encodeResponse(Document response) {
        return Optional.ofNullable(response)
                ...
                .map(this::urlEncode)
                .orElseThrow(() -> new IllegalArgumentException("cannot process empty response document"));
    }
    ...
    private byte[] toByteArray(Document document) {
        ...
        try (Writer writer = new OutputStreamWriter(outputStream, DOCUMENT_CHARSET)) {
            ...
            // how can that be?
            throw new UnhandledException("cannot transform response document to string", e);
        }
    }
